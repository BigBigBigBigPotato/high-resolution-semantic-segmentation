...
class Supervision_Train(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config.weights_name == 'UANet_VGG' or self.config.weights_name == 'UANet_Res50' \
            ...
            loss = self.loss(predictions[0], mask) + self.loss(predictions[1], mask) + self.loss(predictions[2],
                                                                                                 mask) + self.loss(
                predictions[3], mask) + self.loss(predictions[4], mask)
        ...
        pre_mask = pre_mask.argmax(dim=1)  # predict[5]~predict[0]生成的mask图
        for i in range(mask.shape[0]):  # mask是否是多通道？
    ...
    def training_epoch_end(self, outputs):  # 每一个epoch结束后。计算各类参数
